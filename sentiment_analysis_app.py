...
if uploaded_file is not None:
    if uploaded_file.size > 200 * 1024 * 1024:  # Check file size limit (200 MB)
        st.error("The uploaded file exceeds the 200 MB limit. Please upload a smaller file.")
    else:
        file_content = uploaded_file.read().decode("utf-8")
        st.text_area("File Content", file_content, height=200, disabled=True)
        user_input = file_content  # Use file content as input
else:
    # Input text area
    user_input = st.text_area("Enter text to analyze", placeholder="Type something here...")
...
# Footer
st.write("Made with ❤️ from WIZZARDS")
